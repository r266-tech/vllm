    ...
    def __post_init__(self):
        if self.prefix_caching_hash_algo in ("xxhash", "xxhash_cbor"):
            try:
                ...
                raise ImportError(
                    f"prefix_caching_hash_algo="
                    f"'{self.prefix_caching_hash_algo}' requires the "
                    f"xxhash package. Install it with: "
                    f"`pip install xxhash`."
                ) from None

            if not hasattr(_xxhash, "xxh3_128_digest"):
                raise ImportError(
                    "xxhash is installed but too old — "
                    "xxh3_128_digest is not available. "
                    "Please upgrade: `pip install -U xxhash`."
                )
